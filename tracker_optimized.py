...
try:
    # Avoid CPU oversubscription on Raspberry Pi (OpenCV + TFLite threads).
    cv2.setNumThreads(1)
except Exception:
    pass
...
class ThreadedVideoCapture:
    ...
    def read(self) -> Tuple[bool, Optional[np.ndarray]]:
        ...
        full-frame copy per read. If you need an immutable snapshot, call
        ``frame.copy()`` on the returned array.
        """
        with self.lock:
            return self.ret, self.frame
    
    def read_direct(self) -> Tuple[bool, Optional[np.ndarray]]:
        """Alias for :meth:`read` (kept for compatibility)."""
        return self.read()
...
class OptimizedTracker:
    """
    Optimized human tracker for Raspberry Pi 3B/5.
    Target: 24+ stable FPS with detection and tracking.
    """
    ...
    def __init__(self,
                 ...
                 ev3_command_cooldown: float = 0.1):
    ...
    def _run_detection(self, frame: np.ndarray) -> Optional[Dict[str, Any]]:
        ...
        # Use pre-allocated buffer to avoid per-frame heap allocation
    ...
    def _draw_overlay(self, frame: np.ndarray, tracked: Optional[Dict]) -> np.ndarray:
        ...
        if tracked:
            ...
            self.shift_logger.log(shift_x, shift_y)
...
def main():
    ...
    # Detection settings (tuned for Pi performance)
    ...
    parser.add_argument('--ev3-cooldown', type=float, default=0.1,
