...
class Motor:
    ...
    def run(self, direction: int = 1, speed: int = 80):
        """Run motor continuously at given speed (latest-wins)."""
        if self._motor.busy:
            self._motor.stop()
        self._motor.start_move(direction=direction, speed=speed)

    def run_to(self, degrees: int, speed: int = 80):
        """Rotate motor by degrees (latest-wins: stop a prior move first)."""
        if self._motor.busy:
            self._motor.stop()
...
class EV3_USB:
    ...
    def close(self):
        """Close the USB connection to the EV3 brick."""
        if self._ev3 is None:
            return
        ev3 = self._ev3
        self._ev3 = None
        for name in ('close', 'disconnect'):
            fn = getattr(ev3, name, None)
            if callable(fn):
                try:
                    fn()
                    return
