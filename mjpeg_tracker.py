...
from ev3_usb import EV3_Bluetooth
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('mjpeg_tracker.log'),
...
class EV3Controller:
    ...
    def connect(self):
        """Connect to EV3 brick via Bluetooth"""
        try:
            logger.info(f"Connecting to EV3 at {self.mac_address}...")
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    self.ev3 = EV3_Bluetooth(macaddres=self.mac_address)
                    ...
                    self.motor_a = self.ev3.Motor('A')  # Use lowercase
                    time.sleep(0.5)

                    logger.info("Initializing Motor B (Vertical)...")
                    self.motor_b = self.ev3.Motor('B')  # Use lowercase
                    time.sleep(0.5)
            ...
            logger.error(f"Failed to connect to EV3 after {max_retries} attempts: {e}")
            logger.error("Please check:")
            logger.error("  1. EV3 is turned on")
            logger.error("  2. Bluetooth is enabled on EV3")
            logger.error(f"  3. MAC address is correct: {self.mac_address}")
            logger.error("  4. EV3 is paired with this device")
            logger.error("  5. Motors are connected to ports A and B")
            self.connected = False
    ...
    def calculate_motor_direction_speed(self, shift, deadzone, invert=False):
        ...
        # Proportional control: speed increases with distance from center
        # Using a more gradual curve for smoother control
        speed = min(abs(shift) / 5.0 * self.speed_factor, self.max_speed)

        # Ensure minimum speed for movement
        if speed > 0:
            speed = max(speed, 15)  # Minimum speed to overcome motor friction
    ...
    def update_motors(self, shift_x, shift_y):
        ...
        # Throttle commands to avoid overwhelming the EV3
        current_time = time.time()
        if current_time - self.last_command_time < self.command_cooldown:
            return

        try:
            # Calculate horizontal motor (A) direction and speed
            dir_a, speed_a = self.calculate_motor_direction_speed(
                shift_x, self.deadzone_x, self.invert_x
            )

            # Calculate vertical motor (B) direction and speed
            dir_b, speed_b = self.calculate_motor_direction_speed(
                shift_y, self.deadzone_y, self.invert_y
            )
...
class MJPEGTracker:
    def __init__(self, stream_url, output_dir="recordings",
                 ...
                 ev3_mac=None, ev3_deadzone_x=50, ev3_deadzone_y=50,
        ...
        if self.ev3_controller and self.ev3_controller.connected:
            logger.info(f"EV3 control enabled - MAC: {ev3_mac}")
    ...
    def detect_humans_haarcascade(self, frame):
        ...
        bodies = self.body_cascade.detectMultiScale(
            gray,
            scaleFactor=1.15,  # Larger steps = faster
            minNeighbors=3,    # Fewer neighbors = faster but less accurate
            minSize=(30, 30)   # Minimum size filter
        )
    ...
    def init_tracker(self, frame, bbox):
        """Initialize tracker with KCF (faster than CSRT on RPi)"""
        try:
            # KCF is much faster than CSRT and sufficient for most cases
            self.tracker = cv2.legacy.TrackerKCF_create()
            self.tracker.init(frame, bbox)
            return True
        except:
            try:
                # Fallback to MOSSE (fastest tracker)
    ...
    def run_detection_update(self, frame):
        ...
        if detected_human:
            ...
            if self.init_tracker(frame, bbox):
                ...
                logger.debug(f"Human detected and tracked: confidence={detected_human['confidence']:.2f}")
                return self.tracked_human
...
def main():
    ...
    parser.add_argument('--process-scale', type=float, default=0.4,
    ...
    # EV3 arguments
    parser.add_argument('--ev3-mac', type=str, default="00:16:53:40:42:CB",
                        help='EV3 Bluetooth MAC address (e.g., 00:16:53:40:42:CB)')
    parser.add_argument('--ev3-deadzone-x', type=int, default=50,
                        help='Horizontal deadzone in pixels (no motor movement within this range)')
    parser.add_argument('--ev3-deadzone-y', type=int, default=50,
    ...
    parser.add_argument('--ev3-max-speed', type=int, default=30,
