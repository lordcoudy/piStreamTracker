...
class StreamTracker:
    def __init__(self, stream_url, output_dir="recordings",
                 model="deploy.prototxt", weights="res10_300x300_ssd_iter_140000.caffemodel", confidence_threshold=0.5,
        ...
        logger.info(f"Stream Tracker initialized with URL: {stream_url}")
    ...
    def initialize_detection(self):
        try:
            ...
            logger.error(f"Failed to load model files. Make sure '{self.model}' and '{self.weights}' are in the same directory.")
    ...
    def detect_face_dnn(self, frame):
        ...
        for i in range(0, detections.shape[2]):
            ...
            if confidence > self.confidence_threshold:
                # Check if this is the most confident detection so far
    ...
    def detect_single_human(self, frame):
        return self.detect_face_dnn(frame)
    ...
    def run_detection_update(self, frame):
        ...
        small_frame = cv2.dnn.blobFromImage(cv2.resize(frame, (300, 300)), 1.0,
        ...
        if detected_human:
            ...
            bbox = detected_human['bbox'] * np.array([frame.shape[1], frame.shape[0], frame.shape[1], frame.shape[0]])
...
def main():
    ...
    parser.add_argument('--face-model-path', default='deploy.prototxt',
                        help='Path to the Caffe model prototxt file')
    parser.add_argument('--face-model-weights', default='res10_300x300_ssd_iter_140000.caffemodel',
                        help='Path to the Caffe model weights file')
    ...
    tracker = StreamTracker(
        ...
        # Paths to the Caffe model files you downloaded
        model = args.face_model_path,
        weights = args.face_model_weights,
